...
const path = require('path');
const axios = require('axios');
const { cloudinary } = require('../config/cloudinary'); // ✅ Cloudinary only
  ...
  try {
    ...
    pdfDoc.on('end', async () => {
      try {
        ...
        return res.status(500).render('fail', {
          message: 'Failed to save certificate: ' + err.message
        });
      }
    });
    ...
    if (profileImageBuffer) {
      try {
        ...
        console.warn('Profile image in PDF error:', err.message);
      }
    }
...
exports.cancelRegistration = async (req, res) => {
  try {
    ...
    // 2. Delete PDF from local storage
    const fileName = `${userId}-${eventId}.pdf`;
    ...
    try {
      ...
      console.warn('PDF delete warning:', err.message);
    }
